...
if page == L["pages"][0]:  # KPI Snapshot
    ...
    recent_news["competitor"] = recent_news["competitor"].astype(str).str.strip().str.lower()
    funding_df["_company_key"] = funding_df["Company"].astype(str).str.strip().str.lower()
    trials_12m = recent_news[recent_news["tag"].str.contains("trial", case=False, na=False)]\
        .groupby("competitor").size().reset_index(name="Recent Trials (12M)")
    partners_12m = recent_news[recent_news["tag"].str.contains("partner", case=False, na=False)]\
        .groupby("competitor").size().reset_index(name="Partnerships (12M)")
    funding_df = funding_df.merge(trials_12m, how="left", left_on="_company_key", right_on="competitor")
    funding_df = funding_df.merge(partners_12m, how="left", left_on="_company_key", right_on="competitor")
